...
function NonVeggie() {
    const[NonVeggie,setNonVeggie]=useState([])

    useEffect(()=>{
        getNonVeggie();
    },[]);

    const getNonVeggie = async() => {
            const api = await fetch(`https://api.edamam.com/api/recipes/v2?type=public&app_id=${process.env.REACT_APP_APP_ID}&app_key=${process.env.REACT_APP_APP_KEY}&tag=meat`);
            const data = await api.json();
            setNonVeggie(data.hits);
    };
